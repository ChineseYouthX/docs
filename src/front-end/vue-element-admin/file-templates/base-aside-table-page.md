...
<template>
  <div class="${ComponentName}">
    <base-container :has-aside="true">
      <template #aside>
        <base-tree @clickTree="handleClickTreeNode" />
      </template>
      <base-container box-padding="0">
        <template #header>
          <el-row>
            <el-col :span="4">
...
export default {
  ...
  watch: {
    currentDepartmentNode: {
      ...
      handler(node) {
        if ("id" in node) {
          this.requestSearch.departmentId = node.id
          this.getTableData()
        }
      }
    }
